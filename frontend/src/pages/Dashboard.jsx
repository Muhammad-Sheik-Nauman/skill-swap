        ...
        <div className="flex-1 p-6 overflow-y-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 max-h-[900px] overflow-y-auto pr-2 m-6">
            {skills.map((skill, idx) => (
              <div
                key={idx}
                className="bg-white p-5 rounded-xl shadow-md hover:shadow-lg transition transform hover:-translate-y-1 m-6"
              >
                ...
                <div>
                <button className="inline-block text-xs bg-indigo-100 text-indigo-600 px-3 py-1 rounded-full">
                  Learn More
                </button>
                </div>
              </div>
